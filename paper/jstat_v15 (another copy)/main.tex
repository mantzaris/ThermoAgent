...
Large-language-model (LLM) agents place a generative model inside a recurrent
loop of observation, decision, action, and environmental feedback.  This
changes the scientific object from a response to one prompt into a stochastic
process whose future inputs depend on its earlier outputs.  Agent surveys
identify planning, memory, tool use, and interaction with an environment as
recurring architectural components \cite{wang2024agents}; multi-agent systems
add communication protocols, role assignments, and collective task structures
\cite{guo2024multiagents}.  When many such instances interact, even fixed
model weights can generate population trajectories that depend on message
content, network position, update order, retained context, and decoding
randomness.  These dependencies make interacting agents scientifically
important beyond conventional single-prompt evaluation: the system can carry
history, mediate influence through a graph, and develop organization that is
not visible in any isolated response.

Recent agent architectures demonstrate both the opportunity and the source of
that complexity.  Role-playing agents can cooperate through natural-language
exchange \cite{li2023camel}, while generative agents combine observation,
planning, reflection, and a persistent natural-language memory to produce
coordinated social behavior in a simulated environment
\cite{park2023generative}.  Such studies establish that an LLM can be embedded
in an autonomous or socially interactive loop and that retrieved history can
materially condition later actions.  Much of this literature is organized
around task completion, application architecture, or the plausibility of
individual and social behavior.  Those objectives are valuable, but they do
not by themselves identify a controlled stochastic process or determine which
population-level coordinates retain information about its dynamics.

LLM agents are also being used in computational social simulation.  Conditioned
models can reproduce portions of survey-response distributions
\cite{argyle2023}, and simulated samples have replicated several established
behavioral experiments while also revealing model-specific distortions
\cite{aher2023}.  A broader review documents applications of LLM-empowered
agent-based simulation across social, physical, cyber, and hybrid settings
\cite{gao2024abm}.  Networked LLM agents have been used more specifically to
study opinion evolution, where an intrinsic preference for scientifically
accurate answers can favor consensus and prompt-induced confirmation bias can
produce fragmentation \cite{chuang2024}.  This body of work shows that
language-conditioned agents can support controlled in-silico experiments, but
it also cautions against assuming that their trajectories are faithful proxies
for human populations or that semantic plausibility supplies a dynamical
explanation.

Population experiments have begun to target collective behavior directly.
Decentralized LLM populations can form shared naming conventions, amplify
otherwise weak individual preferences, and respond to committed minorities
\cite{flintashery2025}; repeated-game experiments reveal distinct cooperation
and coordination signatures across models and prompts \cite{akata2025}.
Recent statistical-physics studies further show that group size can change
basins of collective misalignment \cite{flint2025}, network topology and the
balance of self and social information can alter conformity
\cite{han2026}, fitted intrinsic fields can dominate neighbor coupling
\cite{denobili2026}, debate noise can produce model-dependent finite-size
ordering crossovers \cite{okawa2026}, and high-dimensional interaction data can
sometimes be summarized by a small family of dynamical regimes
\cite{el2026}.  Together these results establish that collective outcomes are
measurable and strongly protocol- and model-dependent.  They also make clear
that a consensus or coordination score alone does not resolve response,
fluctuation, memory, spatial organization, and temporal ordering.

This setting connects naturally, but not literally, to established
interacting-agent and statistical-physics research.  Agent-based models study
how local rules and heterogeneous interactions generate system-level behavior
\cite{bonabeau2002}; statistical physics has supplied quantitative accounts of
opinion, cultural, language, and consensus dynamics on networks
\cite{castellano2009,baronchelli2018}.  The kinetic Ising model is a canonical
example in which stochastic local updates generate collective relaxation
\cite{glauber1963}, and logit dynamics has an equilibrium stationary law when
interactions derive from a potential \cite{blume1993}.  Nonreciprocal
interactions can instead support structures and responses unavailable to
reciprocal potential systems \cite{fruchart2021}.  These precedents
motivate measurements of order, correlations, fluctuations, response, and
relaxation; they do not imply that an LLM network is a physical material or
that its decoding parameter is a thermodynamic temperature.

Nonequilibrium methods provide a second relevant distinction.  State-space
currents and trajectory reversal can quantify time asymmetry in fully observed
Markov processes \cite{seifert2012,roldan2012}.
An observed LLM belief--action process need not satisfy that closure: prompts
can contain bounded private history and semantically structured packets that
are absent from the measured projection.  Coarse-graining can hide entropy
production and can make inferred lower bounds estimator-dependent
\cite{kawaguchi2013,busiello2019}.  Path reversal is therefore
useful here as a projected temporal-asymmetry diagnostic, not as an assertion
of exact thermodynamic entropy production.  The same caution applies to
finite-window autocorrelation and Binder statistics, which describe
persistence and distribution shape without establishing equilibrium
correlation times, critical slowing down, or a phase transition.

The closest LLM-population studies thus provide important evidence for
conventions, conformity, finite-size ordering, and reduced dynamical laws.
Less resolved is how a controlled network responds when an external field is
reversed and restored, how persistent private history changes the observed
path law relative both to a current-state prompt and to a format-matched
history placebo, and whether a low-dimensional kinetic description closes
those responses out of sample.  Addressing this narrower gap requires a design
that distinguishes the complete simulator state from its measured projection,
enforces local information boundaries, pairs intervention and control
trajectories, and treats independent graph--environment realizations rather
than individual updates as the inferential units.

We therefore combine complementary measurements rather than treating mean
order as a sufficient summary.  Belief and action order, overlap, an effective
compatibility coordinate, and fluctuations describe collective alignment;
entropy and dependence distinguish local uncertainty from synchronous
organization.  Connected graph-distance correlation records spatial
reorganization, while autocorrelation and Binder diagnostics describe
finite-window persistence and order-parameter shape.  Coarse-grained block
reversal probes temporal ordering under Markovized, persistent, and
scrambled-history conditions.  A field quench and restoration supplies a
controlled perturbation; matched controls and exact cluster inference separate
that perturbation from nominal evolution.  Finally, an out-of-sample kinetic
surrogate tests which components admit a simple local closure and which remain
history- or model-dependent.

The contribution is not a general-purpose agent architecture or a performance
benchmark.  It is a controlled finite-size study that treats interacting LLM
instances as measurable stochastic components and asks how collective
observables respond to external fields, retained history, control histories,
and coarse-graining.  The direct experiments use two pinned instruction-model
families, $N=16$ reciprocal modular networks, and finite observation windows.
The interpretation is correspondingly effective and model-specific: no
thermodynamic-limit criticality, exact physical entropy-production law,
universality, task benefit, or claim about human social behavior is inferred.
Section~\ref{sec:system-protocol} defines the network and interventions,
Section~\ref{sec:observables-inference} introduces the observables and
inference, and Section~\ref{sec:evidence-hierarchy} distinguishes exploratory,
corrected earlier, and prospective cross-model evidence.  Sections~\ref{sec:quench-results}--\ref{sec:memory-results}
present the quench and memory results, Section~\ref{sec:closure} tests reduced
closure, and Sections~\ref{sec:discussion}--\ref{sec:conclusions} discuss the
...
\resultfigure{figure05_v14_delayed_audit.pdf}{0.94\textwidth}{%
...
\resultfigure{figure08_path_reversal_sensitivity.pdf}{0.94\textwidth}{%
...
\resultfigure{figure12_memory_prompt_balance.pdf}{0.94\textwidth}{%
...
\resultfigure{figure11_surrogate_size_sensitivity.pdf}{0.94\textwidth}{%
