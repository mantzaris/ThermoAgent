...
The sections below proceed from the system and its observables to the evidence hierarchy, direct results, reduced-description test, and interpretation.

\section{State-separated LLM-agent networks and quench protocols}
\label{sec:system-protocol}

\subsection{Augmented, local, and observed state}
\label{sec:state}
...
\subsection{Information boundaries and random-sequential update dynamics}
\label{sec:information-updates}
...
The corresponding authority and private-state checks are recorded in Appendix~\ref{app:authority}.

\subsection{Markovized, persistent, and scrambled-history conditions}
...
Prompt-balance diagnostics for the scrambled-history control are retained in Figure~\ref{fig:prompt-balance} of Appendix~\ref{app:prompt-balance}.

\subsection{Field quench, restoration, and matched controls}
...
\subsection{Order, overlap, effective compatibility, and fluctuations}
...
\subsection{Entropy, dependence, and the rolling macrostate}
\label{sec:entropy-macrostate}
...
\subsection{Coarse-grained pathwise temporal asymmetry}
...
\subsection{Spatial correlation and finite-window persistence diagnostics}
\label{sec:spatial-persistence-observables}
...
\subsection{Nominal geometry, recovery estimand, and exact cluster inference}
...
The exact-test, multiplicity, and cluster-bootstrap details are retained in Appendix~\ref{app:frozen-inference}.

\section{Evidence hierarchy, correction, and frozen study roles}
\label{sec:evidence-hierarchy}

\subsection{Memory discovery and prospective replication roles}
\label{sec:memory-study-roles}

Figure~\ref{fig:memory} keeps discovery, prospective replication, and the V15 cross-model extension in their distinct study roles.
...
\subsection{Corrected V14 quench evidence and audit boundary}
\label{sec:v14-correction}
...
Figure~\ref{fig:v14series} displays the corrected V14 quench time series.
...
Cluster-level recovery and delayed-audit material is retained in Appendices~\ref{app:v14-correction} and~\ref{app:v14-audit}, including Figures~\ref{fig:v14recovery} and~\ref{fig:v14audit}.

\subsection{Frozen V15 cross-model confirmation and control logic}
\label{sec:v15-frozen-design}
...
\subsection{Cross-model field response}
...
\subsection{Restoration and threshold re-entry}
...
\subsection{Spatial reorganization under the quench}
...
\subsection{Finite-window persistence and order-parameter shape}
\label{sec:persistence-results}

Figure~\ref{fig:persistence-binder} in Appendix~\ref{app:persistence-binder} combines the full autocorrelation curve with the
...
\subsection{Persistent versus Markovized history}
...
\csname V15HTwoDisposition\endcsname{}.

H2 asks whether history matters
relative to an
explicit current-state prompt.

\subsection{Genuine versus scrambled-history control}
\label{sec:persistent-scrambled}
...
\csname V15HThreeDisposition\endcsname{}.

H3 is more stringent: it asks whether the
...
The prompt-balance diagnostic in Figure~\ref{fig:prompt-balance} records the scope of the format and approximate-length control.
...
The block-length, pseudocount, and shuffled-floor sensitivities are displayed in Figure~\ref{fig:path-sensitivity} of Appendix~\ref{app:path-sensitivity}.

\subsection{Confirmatory synthesis across H1--H4}
\label{sec:confirmatory-synthesis}

Figure~\ref{fig:effects} places the four frozen effects and their cluster-bootstrap intervals in a single confirmatory synthesis.
...
\subsection{Information retained beyond order}
...
\subsection{Out-of-sample kinetic surrogate}
\label{sec:surrogate}
...
Figure~\ref{fig:surrogate} compares the direct V14 trajectories with the out-of-sample kinetic surrogate.
...
\subsection{Captured response, failure modes, and the\texorpdfstring{\\}{ }size-context boundary}
...
The effective-model size context is displayed in Figure~\ref{fig:surrogate-size} of Appendix~\ref{app:surrogate-size}.

\section{Discussion}
\label{sec:discussion}

\subsection{Literal stochastic process versus effective statistical mechanics}
\label{sec:literal-effective}
...
\subsection{Projected memory as hidden history}
\label{sec:hidden-history}
...
\subsection{Finite-size, model, topology, and estimator limitations}
...
\subsection{Negative results, scope, and future work}
\label{sec:negative-scope}
...
Further work requires more model families, direct
...
\section{Study chronology, V14 correction, and frozen inferential details}
\label{app:study-inference}

\subsection{Training-only threshold correction and historical H3 invalidation}
\label{app:v14-correction}

Figure~\ref{fig:v14recovery} retains the cluster-level recovery audit under training-only thresholds.
...
\subsection{Delayed V14 permutation and observable-deletion audit}
\label{app:v14-audit}
...
Figure~\ref{fig:v14audit} records the delayed dependence and full-pipeline permutation audit.
...
shown explicitly.  These audits were completed after formal outcomes because
of implementation omissions.}{fig:v14audit}

\subsection{Frozen H1--H4 tests, multiplicity, and cluster bootstrap}
\label{app:frozen-inference}
...
\subsection{Rolling dependence and marginal-shift null construction}
...
\subsection{Block-reversal estimator and floor sensitivity}
\label{app:path-sensitivity}
...
Figure~\ref{fig:path-sensitivity} displays the frozen estimator-sensitivity grid.
...
\subsection{Scrambled-history prompt-balance control}
\label{app:prompt-balance}

Figure~\ref{fig:prompt-balance} records the frozen prompt-balance diagnostic.
...
\subsection{Reduced-model size context}
\label{app:surrogate-size}

Figure~\ref{fig:surrogate-size} places the direct-system anchor beside the frozen effective-model size context.
...
\subsection{Autocorrelation construction and truncation}
\label{app:autocorrelation}

The diagnostics below accompany equations~(\ref{eq:autocorrelation}) and~(\ref{eq:binder}) of the main text.  Each
...
three-sweep truncations remain machine-readable sensitivity analyses.

\subsection{Binder trajectory-first and pooled-moment sensitivity}
\label{app:binder-sensitivity}
...
\subsection{Persistence, Binder, and occupancy display}
\label{app:persistence-display}

Figure~\ref{fig:persistence-binder} displays the accompanying persistence, Binder, and occupancy diagnostics.
...
\clearpage
\section{Implementation integrity, authority, privacy, and provenance}
\label{app:provenance}

\subsection{Agent authority and information-boundary checks}
\label{app:authority}
...
\subsection{Arm independence, replay, and artifact integrity}
\label{app:arm-integrity}
...
\subsection{Reconstruction and technical provenance details}
\label{app:technical-provenance}
