...
8201–8208 for nominal false-activation panels, LLM seed 9101, and the unseen
`tri_region_bridge_v2` topology. It is not run until the freeze record exists
and verifies. Thresholds, budget-matched controls, and operating points come
only from the separate validation set.
...
unmeasured setup reserve, and the projected holdout fit within 35 additional
single-GPU hours. CPU-bound training time on the reserved Pod is included.
