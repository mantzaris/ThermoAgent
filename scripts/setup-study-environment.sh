...
if [[ "$($PYTHON_BOOTSTRAP -c 'import platform; print(platform.python_version())')" != "3.12.3" ]]; then
  if command -v python3.12 >/dev/null 2>&1 \
    ...
    echo "The documented study environment requires Python 3.12.3" >&2
    exit 2
  fi
fi
...
# that the original study environment inherited. Check the exact tested build
...
printf '%s\n' "$ROOT" > "$SITE_PACKAGES/thermoagent-jstat.pth"
...
print("ThermoAgent JSTAT environment ready")
...
identity = {
    "reconstruction_label": "publication-semantic-layout",
