...
def _repository_files():
    roots = [
        ROOT / "thermoagent/statmech_llm/corrected_quench",
        ROOT / "configs/statmech_llm/corrected_quench",
        ROOT / "tests/statmech_llm/corrected_quench",
        ROOT / "results/JSTAT/stages/corrected_quench",
        ROOT / "paper/JSTAT",
    ]
    return [
        path
...
def test_no_secret_patterns_or_crlf_in_corrected_quench_text():
    patterns = (
        "jupyter" + "_token=",
        "api" + "_key=",
...
def test_manuscript_compiles_when_sources_are_present():
    paper = ROOT / "paper/JSTAT/main.tex"
    if not paper.exists():
        pytest.skip("JSTAT manuscript not generated yet")
...
def test_all_final_pdfs_have_embedded_fonts_and_extractable_text_when_present():
    pdfs = list((ROOT / "paper/JSTAT/figures").glob("*.pdf"))
    manuscript = ROOT / "paper/JSTAT/main.pdf"
    if manuscript.exists():
        pdfs.append(manuscript)
    if not pdfs:
        pytest.skip("JSTAT PDFs not generated yet")
    for path in pdfs:
