...
from thermoagent.statmech_llm.corrected_quench.simulation import (
...
def test_private_state_memory_inbox_and_outbox_are_isolated():
    agents = make_replication_agents(8, 1414, "disordered")
...
def test_scheduler_never_substitutes_agent_decision():
    ...
    left = run_corrected_quench_trajectory(_fixed("amber"), graph, 1416, 1, 0.8, 0.5, "nominal", [1, 1, 1])
    right = run_corrected_quench_trajectory(_fixed("cobalt"), graph, 1416, 1, 0.8, 0.5, "nominal", [1, 1, 1])
...
def test_partition_reconnects_and_corruption_rate_is_exactly_half_of_senders():
    ...
    partition = run_corrected_quench_trajectory(
    ...
    corrupt = run_corrected_quench_trajectory(
...
def test_matched_conditions_have_identical_opportunities_and_byte_schema():
    ...
    nominal = run_corrected_quench_trajectory(KineticIsingProvider(), graph, 1423, 3, 0.8, 0.5, "nominal", [1, 1, 1])
    field = run_corrected_quench_trajectory(KineticIsingProvider(), graph, 1423, 3, 0.8, 0.5, "field_reversal", [1, 1, 1])
...
def test_seeded_regeneration_is_exact_for_scripted_provider():
    ...
    first = run_corrected_quench_trajectory(KineticIsingProvider(), graph, 1425, 2, 0.8, 0.5, "nominal", [1, 1, 1])
    second = run_corrected_quench_trajectory(KineticIsingProvider(), graph, 1425, 2, 0.8, 0.5, "nominal", [1, 1, 1])
    assert first == second
