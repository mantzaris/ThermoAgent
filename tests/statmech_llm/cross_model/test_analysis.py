...
from thermoagent.statmech_llm.corrected_quench.observables import irreversibility_sensitivity
...
def _protocol():
    return load_yaml(ROOT / "configs/statmech_llm/cross_model/protocol_template.yaml")
        ...
        for cluster_index in range(6):
            ...
            for condition in conditions:
                for sweep in range(1, 46):
                    ...
                    for feature_index, feature in enumerate(MACROSTATE_FEATURES):
                        row[feature] = float(
                            0.1 * feature_index
                            + 0.02 * cluster_index
                            + shift
...
def test_nominal_fit_is_model_stratified_and_excludes_held_out_cluster():
    ...
    changed.loc[selected, list(MACROSTATE_FEATURES)] += 10000.0
