...
from thermoagent.statmech_llm.figures import (
    _collective_correlations,
    _dynamical_persistence_shape,
    _surrogate,
    _style,
    _corrected_quench_audit,
)
...
def test_corrected_quench_audit_figure_uses_existing_total_correlation_metric_names(tmp_path):
    ...
    with warnings.catch_warnings(record=True) as observed:
        ...
        _corrected_quench_audit(ROOT, result, catalog)
    assert not any(
        "No artists with labels" in str(item.message) for item in observed
    )
    source = pd.read_csv(
        result / "figures/source_data/figure10_delayed_audit.csv"
    )
...
def test_collective_extension_figures_are_vector_pdfs_with_source_data(tmp_path):
    ...
    persistence_source = pd.read_csv(
        result / "figures/source_data/figure14_persistence_binder.csv"
    )
    assert {
        "autocorrelation",
...
def test_surrogate_quench_figure_includes_all_shared_observables_and_intervals(
    ...
    shutil.copyfile(
        ROOT
        / "results/JSTAT/stages/cross_model/tables"
        / "corrected_quench_direct_surrogate_trajectories.csv",
    ...
    assert len(catalog) == 1
    pdf = result / "figures/pdf/figure08_direct_surrogate_quench.pdf"
    assert pdf.read_bytes().startswith(b"%PDF")
    source = pd.read_csv(
        result / "figures/source_data/figure08_direct_surrogate_quench.csv"
    )
