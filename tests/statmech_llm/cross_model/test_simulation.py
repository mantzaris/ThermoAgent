...
from thermoagent.statmech_llm.discovery.core import LatentMapping
from thermoagent.statmech_llm.discovery.provider import FunctionalProvider, KineticIsingProvider
from thermoagent.statmech_llm.replication.simulation import build_reciprocal_graph
from thermoagent.statmech_llm.experiment import formal_panel_design
from thermoagent.statmech_llm.analysis import (
    memory_control_balance_audit,
    memory_control_panel_audit,
)
from thermoagent.statmech_llm.simulation import (
    MemoryControlNetwork,
    memory_control_tape,
    run_study_trajectory,
)
from thermoagent.statmech_llm.workflow import load_yaml
...
def test_formal_design_has_six_matched_clusters_per_model_and_new_seeds():
    protocol = load_yaml(ROOT / "configs/statmech_llm/cross_model/protocol_template.yaml")
...
def test_scheduler_does_not_substitute_decision_and_private_state_isolated():
    ...
    amber = run_study_trajectory(_fixed_choice("amber"), **kwargs)
    cobalt = run_study_trajectory(_fixed_choice("cobalt"), **kwargs)
...
def test_field_quench_and_restoration_use_identical_update_tapes():
    ...
    nominal = run_study_trajectory(provider, graph, 15004, 3, "nominal_markovized", 0.8, 0.5, [1, 1, 1])
    field = run_study_trajectory(KineticIsingProvider(), graph, 15004, 3, "field_markovized", 0.8, 0.5, [1, 1, 1])
...
def test_agent_memory_inbox_and_outbox_are_separate_objects():
    from thermoagent.statmech_llm.replication.simulation import make_replication_agents

    agents = make_replication_agents(8, 15011, "disordered")
    graph = build_reciprocal_graph(8, "modular", 15012)
    network = MemoryControlNetwork(agents, graph, LatentMapping.balanced(15013), "persistent_memory", 0.8, 15014)
...
def test_trajectory_state_is_fresh_and_invariant_to_prior_arm_execution():
    ...
    first = pd.DataFrame(run_study_trajectory(KineticIsingProvider(), **arguments))
    ...
    second = pd.DataFrame(run_study_trajectory(shared_provider, **arguments))
