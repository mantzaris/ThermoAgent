...
from thermoagent.statmech_llm.discovery.analysis import _micro_models, _panel_statistics
from thermoagent.statmech_llm.discovery.experiment import _graph_for_panel, formal_panel_design
from thermoagent.statmech_llm.discovery.provider import KineticIsingProvider
from thermoagent.statmech_llm.discovery.simulation import run_trajectory
from thermoagent.statmech_llm.discovery.workflow import load_yaml


def test_panel_analysis_uses_attempted_update_units_and_bias_floors():
    protocol = load_yaml(__import__("pathlib").Path(__file__).resolve().parents[3] / "configs/statmech_llm/discovery/protocol_template.yaml")
