...
from thermoagent.statmech_llm.discovery.experiment import expected_decisions, formal_panel_design
...
def test_formal_decision_count_and_matched_alpha_arms():
    protocol = load_yaml(ROOT / "configs/statmech_llm/discovery/protocol_template.yaml")
...
def test_raw_artifact_patterns_are_ignored():
    text = (ROOT / ".gitignore").read_text(encoding="utf-8")
    assert "results/JSTAT/**/raw/" in text
    assert "paper/JSTAT/*.aux" in text
