...
from thermoagent.statmech_llm.discovery.provider import FunctionalProvider
from thermoagent.statmech_llm.replication.experiment import (
    expected_decisions,
    formal_panel_design,
    graph_for_panel,
...
from thermoagent.statmech_llm.replication.workflow import execution_source_checksum, load_yaml


ROOT = Path(__file__).resolve().parents[3]
def test_formal_design_count_and_declared_work_packages():
    protocol = load_yaml(ROOT / "configs/statmech_llm/replication/protocol_template.yaml")
...
def test_replication_uses_semantic_namespace():
    assert (ROOT / "thermoagent/statmech_llm/replication").is_dir()
    assert (ROOT / "configs/statmech_llm/replication/protocol.yaml").is_file()
