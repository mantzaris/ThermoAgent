...
def test_agents_own_separate_private_memory_inboxes_and_outboxes():
    agents = make_replication_agents(8, 13, "disordered")
...
def test_scheduler_records_provider_choice_without_substitution():
    ...
    amber = run_replication_trajectory(_fixed("amber"), graph, 31, 1, "markovized", 0.8, 0.5, "disordered")
    cobalt = run_replication_trajectory(_fixed("cobalt"), graph, 31, 1, "markovized", 0.8, 0.5, "disordered")
...
def test_partition_removes_cross_community_delivery_and_recovers():
    ...
    rows = run_replication_trajectory(KineticIsingProvider(), graph, 41, 3, "markovized", 0.8, 0.5, "disordered", "network_partition", [1, 1, 1])
...
def test_field_reversal_and_exact_half_sender_corruption_restore():
    ...
    field = run_replication_trajectory(KineticIsingProvider(), graph, 43, 3, "markovized", 0.8, 0.5, "disordered", "field_reversal", [1, 1, 1])
    for row in field:
    ...
    corrupted = run_replication_trajectory(KineticIsingProvider(), graph, 43, 3, "markovized", 0.8, 0.5, "disordered", "message_corruption", [1, 1, 1])
...
def test_matched_conditions_share_schedule_and_message_opportunities():
    ...
    nominal = run_replication_trajectory(KineticIsingProvider(), graph, 47, 3, "markovized", 0.8, 0.5, "disordered", "nominal", [1, 1, 1])
    partition = run_replication_trajectory(KineticIsingProvider(), graph, 47, 3, "markovized", 0.8, 0.5, "disordered", "network_partition", [1, 1, 1])
