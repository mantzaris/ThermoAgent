...
from thermoagent.statmech_llm.discovery.core import LatentMapping, MEMORY_STATES
from thermoagent.statmech_llm.corrected_quench.analysis import (
    _macro_codes,
    exact_sign_flip_p,
    holm_adjust,
    rolling_macrostates,
    ...
    for (model, window), model_window in output.groupby(["model_key", "window_sweeps"]):
        for cluster in sorted(model_window["cluster_id"].unique()):
            ...
            train_values, test_values = _impute_training(
                training[list(MACROSTATE_FEATURES)].to_numpy(float),
                testing[list(MACROSTATE_FEATURES)].to_numpy(float),
            )
        ...
        rows.append(
            {
                ...
                "macrostate_path_length": phase_path_length(
                    group[list(MACROSTATE_FEATURES)].to_numpy(float)
                ),
...
def analyze_formal(
    ...
    workers = max(1, int(os.environ.get("THERMOAGENT_ANALYSIS_WORKERS", "1")))
    ...
    result = (
        ...
        else repository / "results/JSTAT/stages/cross_model"
    )
    ...
    primary = {
        ...
        "nonfinite_primary_features": int(
            np.sum(~np.isfinite(primary_macro[list(MACROSTATE_FEATURES)].to_numpy(float)))
        ),
