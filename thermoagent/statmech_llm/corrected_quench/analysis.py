...
def memory_discovery_replication(repository: Path) -> Tuple[pd.DataFrame, Dict[str, object]]:
    v12 = json.loads(
        (repository / "results/JSTAT/stages/discovery/statistics/primary_results.json").read_text(encoding="utf-8")
    )["memory_effects"]["adjusted_block_kl_nats_per_update"]
    v13_table = pd.read_csv(repository / "results/JSTAT/stages/replication/tables/hypothesis_effects.csv")
...
def _legacy_memory_sensitivity(repository: Path, protocol: Mapping[str, object]) -> pd.DataFrame:
    roots = [
        Path(os.environ.get("THERMOAGENT_REPLICATION_ARTIFACT_ROOT", "/workspace/ThermoAgent-JSTAT-artifacts/replication")),
        Path(os.environ.get("THERMOAGENT_DISCOVERY_ARTIFACT_ROOT", "/workspace/ThermoAgent-JSTAT-artifacts/discovery")),
    ]
    rows: List[Dict[str, object]] = []
...
def _archive_historical_derived_results(result: Path, paper: Path) -> Dict[str, object]:
    ...
    archive = result / "corrections/scientific_audit_1_1/historical_original"
...
def analyze_formal(repository: Path) -> Dict[str, object]:
    ...
    permutation_workers = int(os.environ.get("THERMOAGENT_CORRECTED_QUENCH_PERMUTATION_WORKERS", "1"))
    ...
    for directory in (
        ...
        result / "logs",
        result / "corrections/scientific_audit_1_1",
    ):
        directory.mkdir(parents=True, exist_ok=True)
    historical_archive = _archive_historical_derived_results(
        result, repository / "paper/JSTAT"
    )
    information_columns = [
        "cluster_id",
        "panel_id",
    ...
    correction_path = result / "corrections/scientific_audit_1_1/correction_record.json"
