"""Command-line entry points for the corrected-quench workflow."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ThermoAgent corrected-quench workflow")
    parser.add_argument(
        "command",
    ...
    if args.command == "pilot":
        ...
        settings = load_yaml(repository / "configs/statmech_llm/corrected_quench/engineering.yaml")["pilot"]
        summary_path = artifact_root() / "pilot" / str(settings["attempt_id"]) / "summary.json"  # type: ignore[index]
        if not summary_path.exists():
            raise RuntimeError("corrected-quench engineering pilot has not completed")
        pilot = json.loads(summary_path.read_text(encoding="utf-8"))
        if not bool(pilot["engineering_passed"]):
            raise RuntimeError("corrected-quench engineering integrity failed; formal execution remains locked")
