"""Content-addressed deterministic regeneration of every V14 trajectory."""
...
from thermoagent.statmech_llm.discovery.replay import RecordedDecisionProvider, RecordedDecisionStore, compare_frames

from .experiment import formal_panel_design, graph_for_panel, panel_seed
from .simulation import run_corrected_quench_trajectory
...
def _digests(frame: pd.DataFrame) -> List[str]:
    ...
    if any(len(value) != 64 for value in values):
        raise RuntimeError("V14 transition row lacks a full external record digest")
    return values
...
def replay_formal(repository: Path) -> Dict[str, object]:
    ...
    for panel in formal_panel_design(protocol):
        ...
        regenerated = run_corrected_quench_trajectory(
            provider,
