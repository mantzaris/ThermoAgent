"""Data-derived vector figures for the discovery-stage evidence package."""
...
def _paths(repository: Path) -> Tuple[Path, Path]:
    results = Path(repository) / "results/JSTAT/stages/discovery"
...
def _figure_12(repository: Path, pdf: Path, source: Path) -> None:
    v10 = pd.read_csv(
        repository
        / "results/JSTAT/stages/discovery/tables/nonreciprocal_reference_quadratic_onset.csv"
    )
    panel = _panel_table()
