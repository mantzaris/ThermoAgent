...
def artifact_root() -> Path:
    path = Path(os.environ.get("THERMOAGENT_DISCOVERY_ARTIFACT_ROOT", "/workspace/ThermoAgent-JSTAT-artifacts/discovery")).resolve()
...
def execution_source_checksum(repository: Path) -> str:
    ...
    for root in roots:
        ...
        for path in sorted(item for item in root.rglob("*") if item.is_file()):
            if "__pycache__" in path.parts or path.suffix in (".pyc", ".pyo") or path.name == "protocol.yaml":
                continue
...
def freeze_protocol(repository: Path, pilot_summary: Mapping[str, object]) -> Dict[str, object]:
    repository = Path(repository).resolve()
    template_path = repository / "configs/statmech_llm/discovery/protocol_template.yaml"
    frozen_path = repository / "configs/statmech_llm/discovery/protocol.yaml"
