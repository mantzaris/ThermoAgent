...
from .simulation import CONDITIONS, condition_specification, memory_control_tape, run_study_trajectory
...
def _write_frozen_manifests(repository: Path, protocol: Mapping[str, object]) -> Dict[str, str]:
    ...
    seed_path = repository / "configs/statmech_llm/cross_model/seed_manifest.csv"
    ...
    control_path = repository / "configs/statmech_llm/cross_model/memory_control_manifest.json"
    atomic_json(
        {
...
def freeze_protocol(repository: Path) -> Dict[str, object]:
    repository = Path(repository).resolve()
    frozen_path = repository / "configs/statmech_llm/cross_model/protocol.yaml"
    ...
    destination = repository / "results/JSTAT/stages/cross_model/protocol/protocol.yaml"
    ...
    atomic_json(summary, repository / "results/JSTAT/stages/cross_model/protocol/freeze_summary.json")
    return summary
...
def run_formal_model(repository: Path, model_key: str) -> Dict[str, object]:
    ...
    source_audit = verify_consolidated_source(repository)
    source = str(source_audit["frozen_execution_source_sha256"])
    consolidated_source = str(source_audit["consolidated_source_sha256"])
    ...
    with stage_lock("formal_%s" % model_key):
        for panel in panels:
            ...
            metadata = {
                ...
                "consolidated_source_sha256": consolidated_source,
            }
