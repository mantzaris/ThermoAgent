...
def _discovery_stage(repository: Path) -> List[Dict[str, object]]:
    source = json.loads((repository / "results/JSTAT/stages/discovery/statistics/primary_results.json").read_text(encoding="utf-8"))
    rows = []
...
def analyze_formal(repository: Path) -> Dict[str, object]:
    ...
    result_dir = repository / "results/JSTAT/stages/replication"
    ...
    atomic_csv(_discovery_stage(repository), tables / "discovery_stage_effects.csv")
