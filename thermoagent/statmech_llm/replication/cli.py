"""Command-line entry points for the replication-stage workflow."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ThermoAgent replication-stage workflow")
    ...
    if args.command == "pilot":
        ...
        settings = load_yaml(repository / "configs/statmech_llm/replication/engineering.yaml")["pilot"]
        summary_path = artifact_root() / "pilot" / str(settings["attempt_id"]) / "summary.json"  # type: ignore[index]
        if not summary_path.exists():
            raise RuntimeError("replication engineering pilot has not completed")
