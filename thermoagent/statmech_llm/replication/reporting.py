"""Compact paper-facing reports, PDF QA, and replication-stage verification."""
...
def _repository_files(repository: Path) -> List[Path]:
    roots = [
        repository / "configs/statmech_llm/replication",
        repository / "thermoagent/statmech_llm/replication",
        repository / "tests/statmech_llm/replication",
        repository / "results/JSTAT/stages/replication",
...
def _write_prose(repository: Path, protocol: Mapping[str, object], primary: Mapping[str, object]) -> None:
    root = repository / "results/JSTAT/stages/replication"
...
- Manuscript: `../../../paper/JSTAT/main.tex` and `main.pdf`

```bash
scripts/run-tests.sh tests/statmech_llm/replication
THERMOAGENT_ENABLE_LLM=1 scripts/run-formal-experiment.sh replication
scripts/replay-results.sh replication
scripts/analyze-results.sh replication
...
def build_results(repository: Path) -> Dict[str, object]:
    ...
    root = repository / "results/JSTAT/stages/replication"
    protocol_path = repository / "configs/statmech_llm/replication/protocol.yaml"
    ...
    invalidated = repository / "configs/statmech_llm/replication/protocol_invalidated_initial.yaml"
    if invalidated.exists():
        atomic_bytes(invalidated.read_bytes(), root / "protocol/protocol_frozen_v1.0_invalidated.yaml")
    invalidated_v11 = repository / "configs/statmech_llm/replication/protocol_invalidated_amended.yaml"
...
def verify_package(repository: Path) -> Dict[str, object]:
    repository = Path(repository).resolve()
    root = repository / "results/JSTAT/stages/replication"
    protocol = load_yaml(repository / "configs/statmech_llm/replication/protocol.yaml")
