"""V13 external-artifact, locking, checksum, and protocol-freeze helpers."""
...
def artifact_root() -> Path:
    fallback = "/workspace/ThermoAgent-JSTAT-artifacts/replication"
    path = Path(os.environ.get("THERMOAGENT_REPLICATION_ARTIFACT_ROOT", fallback)).resolve()
    repository = repository_root()
    if path == repository or repository in path.parents:
        raise ValueError("V13 raw artifacts must remain outside the repository")
    return path
...
def _source_paths(repository: Path) -> List[Path]:
    ...
    paths.extend(path for path in (repository / "scripts").glob("*v13*") if path.is_file())
...
def execution_source_checksum(repository: Path) -> str:
    ...
    for path in _source_paths(repository):
        if "__pycache__" in path.parts or path.suffix in (".pyc", ".pyo") or path.name.startswith("protocol_frozen"):
            continue
...
def atomic_csv(rows: Sequence[Mapping[str, object]], destination: Path) -> None:
    if not rows:
        raise ValueError("refusing to write an empty V13 table")
    fieldnames: List[str] = []
    for row in rows:
        for key in row:
...
def stage_lock(stage: str) -> Iterator[None]:
    ...
    try:
        ...
        raise RuntimeError("exclusive V13 stage lock exists: %s" % path) from error
    try:
...
def freeze_protocol(repository: Path, pilot_summary: Mapping[str, object]) -> Dict[str, object]:
    repository = Path(repository).resolve()
    template_path = repository / "configs/statmech_llm/replication/protocol_template.yaml"
    ...
    atomic_bytes(payload, frozen_path)
    return {
        "protocol_path": str(frozen_path),
        "protocol_sha256": sha256_file(frozen_path),
