...
def _repository_files(repository: Path) -> List[Path]:
    ...
    for filename in ("README.md", "LICENSE", "pyproject.toml", ".gitattributes"):
...
def _effect(effects: pd.DataFrame, hypothesis: str) -> pd.Series:
    ...
    if len(selected) != 1:
        raise RuntimeError("expected one final-study effect for %s" % hypothesis)
...
def build_results(repository: Path) -> Dict[str, object]:
    ...
    extension_path = repository / "configs/statmech_llm/cross_model/collective_extension.yaml"
    extension = load_yaml(extension_path)
    v14_primary = json.loads(
        (
            repository
            / "results/JSTAT/stages/corrected_quench/statistics/primary_results.json"
    ...
    for name, row in extension_macros.items():
        extension_macro_lines.extend(
            [
                r"\newcommand{\Study%s}{%.5f}" % (name, float(row.estimate)),
                r"\newcommand{\Study%sCI}{%.5f to %.5f}"
...
PYTHON_BIN=.venv/bin/python scripts/run-tests.sh
THERMOAGENT_ENABLE_LLM=1 scripts/run-formal-experiment.sh qwen
THERMOAGENT_ENABLE_LLM=1 scripts/run-formal-experiment.sh granite
...
THERMOAGENT_ENABLE_LLM=1 THERMOAGENT_AUTHORIZED_GPU_HOURS=50 scripts/run-formal-experiment.sh qwen
THERMOAGENT_ENABLE_LLM=1 THERMOAGENT_AUTHORIZED_GPU_HOURS=50 scripts/run-formal-experiment.sh granite
...
\newcommand{{\StudyTrajectories}}{{{int(completion['dynamic_trajectories'])}}}
\newcommand{{\StudyDecisions}}{{{int(completion['observed_decision_rows']):,}}}
\newcommand{{\StudyClustersPerModel}}{{{int(primary['independent_clusters_per_model'])}}}
\newcommand{{\StudyGPUHours}}{{{total_hours:.2f}}}
\newcommand{{\StudyOriginalGPUHours}}{{{historical_total_hours:.2f}}}
\newcommand{{\StudyReconstructionGPUHours}}{{{total_hours:.2f}}}
\newcommand{{\StudyOrphanFormalDecisions}}{{{orphan_decisions}}}
\newcommand{{\StudyOrphanFormalGPUHours}}{{{orphan_hours:.3f}}}
\newcommand{{\StudyUnrecordedInfrastructureCalls}}{{{int(unrecorded['model_calls'])}}}
\newcommand{{\StudyHOne}}{{{float(h1.estimate):.3f}}}
\newcommand{{\StudyHOneCI}}{{{float(h1.ci_low):.3f} to {float(h1.ci_high):.3f}}}
\newcommand{{\StudyHTwo}}{{{float(h2.estimate):.5f}}}
\newcommand{{\StudyHTwoCI}}{{{float(h2.ci_low):.5f} to {float(h2.ci_high):.5f}}}
\newcommand{{\StudyHThree}}{{{float(h3.estimate):.5f}}}
\newcommand{{\StudyHThreeCI}}{{{float(h3.ci_low):.5f} to {float(h3.ci_high):.5f}}}
\newcommand{{\StudyHFour}}{{{float(h4.estimate):.3f}}}
\newcommand{{\StudyHFourCI}}{{{float(h4.ci_low):.3f} to {float(h4.ci_high):.3f}}}
\newcommand{{\StudyHOneDisposition}}{{{'supported' if disposition['H1']['supported'] else 'not supported'}}}
\newcommand{{\StudyHTwoDisposition}}{{{'supported' if disposition['H2']['supported'] else 'not supported'}}}
\newcommand{{\StudyHThreeDisposition}}{{{'supported' if disposition['H3']['supported'] else 'not supported'}}}
\newcommand{{\StudyHFourDisposition}}{{{'supported' if disposition['H4']['supported'] else 'not supported'}}}
{extension_macro_text}
"""
    paper = repository / "paper/JSTAT"
    paper.mkdir(parents=True, exist_ok=True)
    atomic_bytes(macros.encode("utf-8"), paper / "study_results.tex")
    external = {
...
def validate_pdfs(repository: Path) -> Dict[str, object]:
    repository = Path(repository).resolve()
    result = repository / "results/JSTAT/stages/cross_model"
    pdfs = sorted((repository / "paper/JSTAT/figures").glob("*.pdf"))
    pdfs.extend(
        path
        for path in (
    ...
    if not pdfs:
        raise RuntimeError("no JSTAT study PDFs found")
...
def verify_package(repository: Path) -> Dict[str, object]:
    ...
    v14_primary = json.loads(
        (repository / "results/JSTAT/stages/corrected_quench/statistics/primary_results.json").read_text(encoding="utf-8")
    )
    ...
    checks = {
        ...
        == sha256_file(repository / "configs/statmech_llm/cross_model/collective_extension.yaml"),
