...
from thermoagent.statmech_llm.discovery.core import LatentMapping, MEMORY_STATES, StructuredProvider
from thermoagent.statmech_llm.discovery.graphs import DeliveryGraph
from thermoagent.statmech_llm.discovery.simulation import DecentralizedStatmechNetwork, UpdateTape, generate_update_tape
from thermoagent.statmech_llm.replication.observables import instantaneous_state
from thermoagent.statmech_llm.replication.simulation import build_reciprocal_graph, make_replication_agents, phase_for_update
...
class MemoryControlNetwork(DecentralizedStatmechNetwork):
...
def memory_control_tape(
    ...
    agents = make_replication_agents(int(n_agents), int(panel_seed), "disordered")
    graph = build_reciprocal_graph(int(n_agents), "modular", int(panel_seed) + 7001)
    network = MemoryControlNetwork(
        agents,
        graph,
        mapping,
        "scrambled_memory",
