"""Compact publication package, PDF QA, and final V12 integrity checks."""
...
def _results(repository: Path) -> Path:
    return Path(repository) / "results/llm_agent_statmech_v12"
...
def _repository_manifest(repository: Path) -> List[Dict[str, object]]:
    roots = [_results(repository), repository / "paper/jstat_v12"]
    rows: List[Dict[str, object]] = []
...
def build_results(repository: Path) -> Dict[str, object]:
    ...
    protocol_path = repository / "configs/statmech_v12/protocol_frozen.yaml"
...
def validate_pdfs(repository: Path) -> Dict[str, object]:
    ...
    manuscript = repository / "paper/jstat_v12/main.pdf"
