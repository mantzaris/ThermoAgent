...
def artifact_root() -> Path:
    fallback = "/workspace/ThermoAgent-v13-artifacts"
    path = Path(os.environ.get("THERMO_V13_ARTIFACT_ROOT", fallback)).resolve()
...
def freeze_protocol(repository: Path, pilot_summary: Mapping[str, object]) -> Dict[str, object]:
    repository = Path(repository).resolve()
    template_path = repository / "configs/statmech_v13/protocol_template.yaml"
    frozen_path = repository / "configs/statmech_v13/protocol_frozen_v1.2.yaml"
