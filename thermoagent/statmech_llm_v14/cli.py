"""Command-line entry points for the staged V14 workflow."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ThermoAgent V14 memory and quench statistical mechanics")
    parser.add_argument(
        "command",
    ...
    if args.command == "pilot":
        ...
        settings = load_yaml(repository / "configs/statmech_v14/engineering.yaml")["pilot"]
        summary_path = artifact_root() / "pilot" / str(settings["attempt_id"]) / "summary.json"  # type: ignore[index]
        if not summary_path.exists():
            raise RuntimeError("V14 engineering pilot has not completed")
        pilot = json.loads(summary_path.read_text(encoding="utf-8"))
        if not bool(pilot["engineering_passed"]):
            raise RuntimeError("V14 engineering integrity failed; formal execution remains locked")
