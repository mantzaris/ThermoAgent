...
from thermoagent.statmech_llm_v12.provider import (
    MODEL_ID,
    MODEL_REVISION,
    QwenStatmechProvider,
    schema_checksum,
)
from thermoagent.statmech_llm_v12.replay import RecordedDecisionProvider, RecordedDecisionStore

from .simulation import build_reciprocal_graph, run_v14_trajectory
...
def _require_qwen_opt_in() -> None:
    if os.environ.get("THERMO_V14_ENABLE_QWEN") != "1":
        raise RuntimeError("V14 Qwen execution requires THERMO_V14_ENABLE_QWEN=1 on the authorized Pod")
...
def run_engineering_pilot(repository: Path) -> Dict[str, object]:
    ...
    configuration = load_yaml(repository / "configs/statmech_v14/engineering.yaml")
...
def run_formal_experiment(repository: Path) -> Dict[str, object]:
    _require_qwen_opt_in()
    repository = Path(repository).resolve()
    frozen_path = repository / "configs/statmech_v14/protocol_frozen.yaml"
