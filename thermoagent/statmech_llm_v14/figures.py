...
def _architecture(name: str, source_dir: Path, pdf_dir: Path) -> None:
    ...
    for row in edges.itertuples():
        ...
        ax.annotate("", xy=(end.x, end.y), xytext=(start.x, start.y), arrowprops=dict(arrowstyle="->", color="0.35", lw=1.2))
...
def _memory_clusters(name: str, repository: Path, source_dir: Path, pdf_dir: Path) -> None:
    v13 = pd.read_csv(repository / "results/collective_agent_statmech_v13/tables/panel_statistics.csv")
    ...
    v12 = pd.read_csv(repository / "results/llm_agent_statmech_v12/tables/memory_effects.csv")
...
def _line_groups(name: str, frame: pd.DataFrame, x: str, y: str, group: str, xlabel: str, ylabel: str, source_dir: Path, pdf_dir: Path, shade: bool = False) -> None:
    ...
    for value, subset in summary.groupby(group):
        color = COLORS.get(str(value), "#0072B2")
        ax.plot(subset[x], subset["mean"], marker=MARKERS.get(str(value), "o"), label=LABELS.get(str(value), str(value)), color=color)
...
def _phase_portrait(name: str, macro: pd.DataFrame, condition: str, x: str, y: str, xlabel: str, ylabel: str, source_dir: Path, pdf_dir: Path) -> None:
    ...
    for index in (0, 14, 29, 44):
        if index < len(mean): ax.annotate(str(index+1), (mean.iloc[index][x], mean.iloc[index][y]), xytext=(3,3), textcoords="offset points", fontsize=8)
...
def _robustness(name: str, frame: pd.DataFrame, source_dir: Path, pdf_dir: Path) -> None:
    selected = frame[(frame["ablation"] == "all") & (frame["nominal_fit_window"] == "all_nominal")].copy()
...
def _family_ablation(name: str, frame: pd.DataFrame, source_dir: Path, pdf_dir: Path, contribution: bool = False) -> None:
    selected = frame[(frame["disruption"] == "field_reversal") & (frame["nominal_fit_window"] == "all_nominal") & (frame["estimator"] == "shrinkage") & np.isclose(frame["ridge_fraction"],.1)].copy()
    _source(selected, source_dir / f"{name}.csv")
    ...
    fig, ax = plt.subplots(figsize=(6.0, 3.4)); ax.barh(means["ablation"].str.replace("without_","− "), means["plotted_value"], color="#56B4E9")
    ax.axvline(0, color="0.35", lw=0.8)
    ax.set_xlabel(xlabel); ax.set_ylabel("Observable family retained/removed")
    _finish(fig, pdf_dir / f"{name}.pdf")
...
def _v13_topology(name: str, repository: Path, source_dir: Path, pdf_dir: Path) -> None:
    frame = pd.read_csv(repository / "results/collective_agent_statmech_v13/tables/panel_statistics.csv")
...
def _surrogate(name: str, repository: Path, source_dir: Path, pdf_dir: Path) -> None:
    direct=pd.read_csv(repository/"results/collective_agent_statmech_v13/tables/panel_statistics.csv")
    ...
    surrogate=pd.read_csv(repository/"results/collective_agent_statmech_v13/tables/surrogate_phase_map.csv")
...
def _effect_forest(name: str, effects: pd.DataFrame, memory: pd.DataFrame, source_dir: Path, pdf_dir: Path) -> None:
    field=effects.copy(); field["label"]=field["hypothesis"]+": "+field["estimand"]
    mem=memory.iloc[:2].copy(); mem["label"]=mem["study"]; mem["hypothesis"]="H1"
    source=pd.concat([field[["label","hypothesis","estimate","ci_low","ci_high"]],mem[["label","hypothesis","estimate","ci_low","ci_high"]]],ignore_index=True)
...
def generate_figures(repository: Path) -> Dict[str, object]:
    _style()
    repository=Path(repository).resolve(); root=repository/"results/collective_agent_statmech_v14"
