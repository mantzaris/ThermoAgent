...
from thermoagent.statmech_llm_v12.graphs import select_recipient
from thermoagent.statmech_llm_v12.simulation import generate_update_tape
from thermoagent.statmech_llm_v13.observables import instantaneous_state, integrated_correlation_time, plugin_entropy
from thermoagent.statmech_llm_v13.simulation import make_v13_agents, phase_for_update
from thermoagent.statmech_llm_v13.simulation import build_reciprocal_graph
from thermoagent.statmech_llm_v13.surrogate import fit_kinetic_surrogate
from thermoagent.statmech_llm_v14.experiment import formal_panel_design as v14_design
from thermoagent.statmech_llm_v14.experiment import graph_for_panel as v14_graph
from thermoagent.statmech_llm_v14.experiment import panel_seed as v14_panel_seed
from thermoagent.statmech_llm_v14.observables import recovery_time, signed_polygon_area
...
def fit_reference_coefficients(repository: Path) -> Dict[str, object]:
    frame = pd.read_csv(
        Path(repository) / "results/collective_agent_statmech_v13/tables/microscopic_response.csv"
    )
...
def v14_out_of_sample_quench_comparison(repository: Path) -> Dict[str, object]:
    ...
    direct = pd.read_csv(
        repository / "results/collective_agent_statmech_v14/tables/macrostate_trajectories.csv"
    )
    ...
    result = repository / "results/collective_agent_statmech_v15/tables"
    result.mkdir(parents=True, exist_ok=True)
    atomic_csv(combined, result / "v14_direct_surrogate_quench_trajectories.csv")
    atomic_csv(summary_frame, result / "v14_direct_surrogate_quench_summary.csv")
    atomic_csv(thresholds, result / "v14_direct_surrogate_thresholds.csv")
    metadata = {
    ...
    atomic_json(metadata, repository / "results/collective_agent_statmech_v15/statistics/surrogate_fit.json")
    return metadata
...
def dense_surrogate_size_quench(repository: Path) -> Dict[str, object]:
    ...
    atomic_json(metadata, repository / "results/collective_agent_statmech_v15/statistics/surrogate_size_summary.json")
    return metadata
...
__all__ = [
    ...
    "v14_out_of_sample_quench_comparison",
]
